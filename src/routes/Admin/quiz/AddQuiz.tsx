...
export default function AddQuizModal({ setAddQuizModal,
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    ...
    try {
      setApiloading(true)
      // STEP 1: Create Quiz only if not already created
      if (!quizIdRef.current) {
        const quizPayload = {
          ...
          quizLevel: "1",
          quizTag: data.quizTags,
          description: data?.quizDescription,
        };

        const createRes = await createQuiz(quizPayload).unwrap();
        const newQuizId = createRes?.data?.result?.id;
        console.log(createRes, "createRes");

        quizIdRef.current = newQuizId;
      }

      // STEP 2: AI Service to generate questions
      ...
      const fileUploadData = uploadRes?.data?.results?.[0]?.questions;
      if (!fileUploadData || fileUploadData.length === 0) {
        setPopupMessage('AI service did not return any questions')
        setShowPopup(true)
      }

      // STEP 3: Bulk create questions using previously stored quiz ID
      const payloadBulk = {
        quizId: quizIdRef.current,
        data: fileUploadData,
      };

      const res = await createQuizQuestions(payloadBulk);
      if (res?.data?.statusCode == 200) {
        console.log("Quiz and questions created successfully:", res);
        setPopupMessage(res?.data?.status)
        setShowPopup(true)
        setAddQuizModal(false);
        quizIdRef.current = null; // Reset after success
      }
      setApiloading(false)
    } catch (error) {
      const errorMessage = (error as any)?.data?.error?.message || (error as any)?.data?.error || 'Please try again';
  ...
  if (isLoading) {
              ...
              <div className="w-full">
                ...
                >
                  <div className="grid w-full grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="col-span-2">
                      <div className="text-sm mb-2 ml-1">Upload File</div>
    ...
    `}
                    ...
                    <div>
                      <label className="block text-sm mb-2 text-[#4E4E4E]">
                        Quiz Topic <span className="text-sm text-red-500">*</span>
                      </label>
                      <input
                        type="text"
                        placeholder="Enter Quiz Topic"
                        {...register("quizTopic", {
                          required: "Quiz Topic are required",
                        })}
                        className={clsx(
                          "w-full bg-transparent h-[50px] rounded-lg border text-sm px-3 focus:outline-none",
                          errors.quizTopic ? "border-red-500" : "border-[#A0A0A0]"
                        )}
                      />
                      {errors.quizTopic && (
                        <p className="text-red-500 text-sm mt-1">
                          {errors.quizTopic.message}
                        </p>
                      )}
                    </div>
                    <div>
                  ...
                  <div>
                    <label className="block text-sm mb-2 mt-6 text-[#4E4E4E]">
                    ...
                    <textarea
                      ...
                      className={clsx(
                        "w-full bg-transparent h-[100px] rounded-lg border text-sm p-3 focus:outline-none",
                        errors.quizDescription ? "border-red-500" : "border-[#A0A0A0]"
                      )}
                    />
